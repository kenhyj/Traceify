...
router.put('/api/expose',(req,res)=>{
    ...
    {$and:[{'location.lat':{$gt: locations[0].lat-E}}, {'location.lng':{$gt: locations[0].lng-E}}]}]}).toArray()
    .then((result1)=>{
        ...
        return db.get().collection('locationTraces').find({$and:[{date: {$regex: new RegExp('^' + time)}},
        {$and:[{'location.lat':{$gt: locations[1].lat-E}}, {'location.lng':{$gt: locations[1].lng-E}}]}]}).toArray()
    }).then((result2)=>{
        ...
        return db.get().collection('locationTraces').find({$and:[{date: {$regex: new RegExp('^' + time)}},
        {$and:[{'location.lat':{$gt: locations[2].lat-E}}, {'location.lng':{$gt: locations[2].lng-E}}]}]}).toArray()
    }).then((result)=>{
