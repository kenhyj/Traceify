...
router.put('/api/expose', (req, res) => {
    ...
    return db.get().collection('locationTraces').find({
        $and: [{ 'date': { $regex: new RegExp('^' + time) } },
        {
            $and: [{ 'location.lat': { $gt: locations[0].lat - E } }, { 'location.lat': { $lt: locations[0].lat + E } },
            { 'location.lng': { $gt: locations[0].lng - E } }, { 'location.lng': { $lt: locations[0].lng + E } }]
        }]
    }).toArray()
        .then((result1) => {
            ...
            return db.get().collection('locationTraces').find({
                $and: [{ 'date': { $regex: new RegExp('^' + time) } },
                {
                    ...
                    { 'location.lng': { $gt: locations[1].lng - E } }, { 'location.lng': { $lt: locations[1].lng + E } }]
                }]
            }).toArray()
        }).then((result2) => {
            ...
            return db.get().collection('locationTraces').find({
                $and: [{ 'date': { $regex: new RegExp('^' + time) } },
                {
                    ...
                    { 'location.lng': { $gt: locations[2].lng - E } }, { 'location.lng': { $lt: locations[2].lng + E } }]
                }]
            }).toArray()
        }).then((result) => {
            ...
            return db.get().collection('locationOutbreak').find({
                $and: [{ 'date': { $regex: new RegExp('^' + time) } },
                {
                    $and: [{ 'location.lat': { $gt: locations[0].lat - E } }, { 'location.lat': { $lt: locations[0].lat + E } },
                    { 'location.lng': { $gt: locations[0].lng - E } }, { 'location.lng': { $lt: locations[0].lng + E } }]
            ...
            if (locations[0].loc !== '' && result4.length !== 0) {
                outbreaks.push({
                    place: locations[0].loc,
                    date: time,
                });
            ...
            if (locations[1].loc !== '' && result5.length !== 0) {
                outbreaks.push({
                    place: locations[1].loc,
                    date: time,
                });
            ...
            if (locations[2].loc !== '' && result6.length !== 0) {
                outbreaks.push({
                    place: locations[2].loc,
                    date: time,
                });
