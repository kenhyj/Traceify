...
function handleDelete(selected, trigger, setSelected) {
  axios
    .delete('/api/location-outbreak', { data: selected })
    .then(() => {
...
const EnhancedTableToolbar = (props) => {
  ...
  const { numSelected, selected, trigger, setSelected } = props;
      ...
      {numSelected > 0 ? (
        <Tooltip title='Delete'>
          <IconButton aria-label='delete'>
            <DeleteIcon
              onClick={() => handleDelete(selected, trigger, setSelected)}
            />
          </IconButton>
        </Tooltip>
      ) : (
...
const mapStateToProps = (state) => {
  return {
    locationTraces: state.website.locationTraces,
  };
};

export default withRouter(
  connect(mapStateToProps, { setLocationTraces })(AdminDeleteOutbreak)
);
